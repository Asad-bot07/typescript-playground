...
Create an object car with properties brand, model, and year. Print them.

Write a class Person with properties name and age. Add a method introduce() that returns "Hi, I'm {name}, {age} years old.".

Create a class Student that extends Person and adds a property grade. Override introduce() to also include the grade.

Demonstrate the use of public, private, and protected with a class BankAccount.

🟡 Constructors & Methods

Write a class Rectangle with properties width and height. Add a method getArea() that returns the area.

Create a class Circle with a radius property. Add a method getCircumference().

Create a class Employee with a constructor that initializes id, name, and salary. Add a method getDetails().

🔵 Inheritance & Polymorphism

Write a class Animal with a method makeSound(). Extend it with Dog and Cat, overriding makeSound() differently.

Create a base class Shape with a method area(). Extend it with Square and Triangle, each implementing area() differently.

Demonstrate method overloading in TypeScript with a class Calculator.

🟣 Interfaces & Abstract Classes

Create an interface Vehicle with properties brand and speed. Implement it in a class Car.

Write an abstract class Device with an abstract method powerOn(). Extend it with Laptop and Phone.

Show how an interface can extend another interface with Person and Employee.

🔴 Generics

Create a generic function identity<T>(value: T): T. Test it with string and number.

Write a generic class Box<T> that stores a value and has a method getValue().

Create a generic function getFirst<T>(arr: T[]): T | undefined.

🟤 Modules

Create two files: mathUtils.ts and app.ts. Export a function add(a, b) from mathUtils.ts and import it in app.ts.

Write a default export function greet() in one file and import it in another.

Demonstrate named and default exports together in a small example.
```
